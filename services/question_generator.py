...
1. Ровно 4 варианта ответа для формата "тест" (один правильный, три неправильных)
2. Развёрнутый эталонный ответ (3–5 предложений) для формата "открытый вопрос"

Требования к неправильным вариантам (дистракторам) — КРИТИЧЕСКИ ВАЖНО:
- Дистрактор должен быть утверждением, которое реально существует в предметной области,
  но не является ответом на ДАННЫЙ вопрос (например, описание смежного понятия или механизма)
- Используй типичные заблуждения и ошибки, которые действительно допускают студенты
- Все 4 варианта должны быть ОДИНАКОВЫМИ по длине, стилю, уровню детализации и тону
- Дистрактор НЕ должен содержать слов-маркеров: "всегда", "никогда", "только", "все без исключения"
- Дистрактор НЕ должен быть абсурдным, шуточным или очевидно неверным даже для новичка
- Правильный ответ НЕ должен выделяться: не делай его самым длинным, самым подробным
  или единственным с примером
- Если в вопросе фигурируют числа или термины — дистракторы должны использовать
  реалистичные близкие значения и термины из той же категории
